...
-- WHERE date > (SELECT MAX("date") FROM all_logs);
-- WHERE parse_datetime(CAST(date AS varchar) || ' ' || time, 'yyyy-MM-dd HH:mm:ss') > (SELECT MAX(parse_datetime(CAST(date AS varchar) || ' ' || time, 'yyyy-MM-dd HH:mm:ss')) FROM all_logs)
-- WHERE date = CAST(current_date - interval '1' day AS date);
WHERE date > (SELECT MAX("date") FROM all_logs) AND date <= CAST(current_date - interval '1' day AS date);
